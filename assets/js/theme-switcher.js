	...
	function initStyleSwitcher() {
		...
		function setActiveLink(activeElm) {
			...
			var inactiveElms = document.querySelectorAll(
				'link[title]:not([href*="' + activeHref + '"]):not([title="' + activeTitle + '"])',
			);
			...
			for (var i = 0; i < inactiveElms.length; i++) {
				...
				if (window.browsersyncObserver) {
					...
					var linkRelAlt =
						linkRel.indexOf("alternate") > -1 ? linkRel : (linkRel + " alternate").trim();
		...
		if (!isInitialzed) {
			...
			document.addEventListener("click", function (evt) {
				...
				if (dataHref || dataTitle) {
					dataTitle =
						dataTitle || evt.target.textContent || "_" + Math.random().toString(36).substr(2, 9); // UID
